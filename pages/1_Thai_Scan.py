...
st.markdown("""
    ...
    /* ปรับแต่งปุ่มให้เล็กลงและประหยัดพื้นที่สูงสุด */
    .stButton > button {
    ...
    /* ลดระยะห่าง Column เพื่อไม่ให้ล้น */
    [data-testid="column"] {
...
# แถว 2: แบ่งครึ่งแบบลดระยะห่างสุดๆ
c1, c2 = st.columns(2)
with c1:
...
# แถว 3: แบ่งครึ่งแบบลดระยะห่างสุดๆ
c3, c4 = st.columns(2)
with c3:
...
if cp == "Thai Scan":
    st.markdown(f'<div class="time-status">🕒 {datetime.now(pytz.timezone("Asia/Bangkok")).strftime("%H:%M:%S")} | V6.8 Stable</div>', unsafe_allow_html=True)
    ...
    st.info("PPE Guardian Dashboard\nปรับแก้ปุ่มให้เล็กลงเพื่อไม่ให้ล้นหน้าจอมือถือแล้วครับ")
...
st.write("---")
st.caption("PPE | Ultra Compact v6.8")
