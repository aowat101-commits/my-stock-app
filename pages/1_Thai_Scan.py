...
# --- 2. TICKERS (จากฐานข้อมูลบริษัท Por Piang Electric Plus) ---
...
def analyze_guardian_v4_2(ticker):
    try:
        # ดึงประวัติ 60 วันเพื่อความเสถียรของข้อมูล Prev
        df = yf.download(ticker, period="60d", interval="1h", progress=False)
        if isinstance(df.columns, pd.MultiIndex): df.columns = df.columns.get_level_values(0)
        if df.empty or len(df) < 30: return None
        df = df.dropna()

        # Indicators (HMA, EMA, WaveTrend ตามความเชี่ยวชาญเทคนิค)
        ...
        if not all_sig.empty:
            ...
            # ข้อมูลเปรียบเทียบสำหรับตรรกะสี
...
dashboard()
st.write("---")
st.caption("Por Piang Electric Plus Co., Ltd. | Stable Rollback v4.2")
