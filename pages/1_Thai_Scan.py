...
st.markdown("""
    ...
    .st.DataFrame [data-testid="stTable"] { background-color: #000000 !important; }
    ...
    try:
        ...
        for i in range(len(df)-1, 0, -1):
            ...
            if cp > ema8.iloc[i] and h_curr > h_prev and vol > (v5 * 1.2):
                ...
                f_time = df.index[i].astimezone(pytz.timezone('Asia/Bangkok')).strftime("%H:%M %d/%m"); break
        ...
        c6_val = f"{t_val:.2f}M" if 'W' in mode else s_label
        v_col = "#6b7280" if 'W' in mode else s_col
        if 'W' in mode:
            if t_val > 100: v_col = "#A855F7"
            elif t_val >= 10: v_col = "#06B6D4"
        return [ticker.upper(), f"{c_pp:.2f}", f"{c_cp:.2f}", f"{chg:+.2f}", f"{(chg/c_pp)*100:.2f}%", c6_val, f_time, "#00FF00" if chg > 0 else "#FF1100", v_col, s_col]
    except: return None
...
if p == 'Home':
    ...
    st.write(f'<div class="classic-header">PPE Guardian V9.0 | {dt_str}</div>', unsafe_allow_html=True)

elif p in ['TW', 'UW', 'TS', 'US']:
    c6_n = "Value (M)" if 'W' in p else "Signal"
    ...
    res = [fetch_guardian_engine(t, p) for t in d_l if fetch_guardian_engine(t, p)]
    if res:
        df = pd.DataFrame(res, columns=["Ticker", "Prev", "Price", "Chg", "%Chg", c6_n, "Time Update", "PC", "VC", "TC"])
        st.dataframe(df.style.apply(apply_style, axis=1), use_container_width=True, hide_index=True, column_order=("Ticker", "Prev", "Price", "Chg", "%Chg", c6_n, "Time Update"))
