...
# --- 2. SIDEBAR MENU (ระบบเลือกหน้าแบบ ThaiChart) ---
with st.sidebar:
    st.title("📌 Menu")
    # ตัวเลือกเมนูหน้าหลัก
    app_mode = st.radio("เลือกหน้าที่ต้องการ:", ["หน้าหลัก (Dashboard)", "ข้อมูลย้อนหลัง", "ตั้งค่า"], index=0)
...
# --- 3. TICKERS & ENGINE ---
# (ดึงมาจากประวัติการใช้งานของคุณมิลค์)
...
def analyze_guardian_v4_2(ticker):
    try:
        ...
        # อินดิเคเตอร์ที่ใช้ (HMA, EMA, WaveTrend ตามประวัติการเทรด)
...
# --- 4. MAIN DASHBOARD ---
if app_mode == "หน้าหลัก (Dashboard)":
    st.subheader("🛡️ Guardian Balanced v4.2 (ThaiChart Style)")
    
    tz = pytz.timezone('Asia/Bangkok')
    st.markdown(f'<div class="time-status">🕒 {datetime.now(tz).strftime("%H:%M:%S")} | Mode: Sidebar Navigation</div>', unsafe_allow_html=True)
    ...
    if results:
        ...
        styled = df_d.style.format({"Prev": "{:,.2f}", "Price": "{:,.2f}", "%Chg": "{:+.2f}%"}).apply(get_row_styles, axis=1)
        st.dataframe(styled, use_container_width=True, height=750, hide_index=True)
    else:
        st.warning("⚠️ ไม่พบข้อมูล กรุณากด Refresh ในเมนูด้านซ้าย")

elif app_mode == "ข้อมูลย้อนหลัง":
    st.subheader("📁 ข้อมูลประวัติสัญญาณ")
    st.info("ส่วนนี้ไว้สำหรับดูข้อมูลย้อนหลัง (คุณมิลค์สามารถเพิ่มตารางหุ้นที่เคยซื้อไว้ที่นี่ได้ครับ)")

elif app_mode == "ตั้งค่า":
    st.subheader("⚙️ การตั้งค่าระบบ")
    st.write("จัดการการแจ้งเตือนหรือปรับแต่งกราฟ")
