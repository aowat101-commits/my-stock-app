...
# --- 1. ปรับ CSS ให้รองรับการ Render กราฟบนมือถือได้ดีขึ้น ---
st.set_page_config(page_title="Guardian Alpha Mobile", layout="wide")
st.markdown("""
    <style>
    [data-testid="stStatusWidget"] {display: none !important;}
    [data-testid="stTableDataHeaderCell"], th { pointer-events: none !important; }
    button[title="Sort column"], .st-emotion-cache-1pxm6y3, [data-testid="stHeaderActionElements"] { display: none !important; }
    [data-testid="stDataFrame"] td, [data-testid="stDataFrame"] th { font-size: 10.5px !important; padding: 2px !important; }
    .stDataFrame { height: 420px; }

    /* ปรับปรุง Overlay ให้ Chrome Android โหลดกราฟติด */
    .mobile-overlay {
        position: fixed; top: 0; left: 0; width: 100%; height: 100%;
        background-color: #0f172a; z-index: 9999; overflow-y: auto; padding: 10px;
    }
    </style>
    """, unsafe_allow_html=True)

# --- (ส่วนฟังก์ชันข้อมูลคงเดิมตามเวอร์ชันเสถียรของคุณ) ---
@st.cache_data
def get_slim_test_20():
    ...
    for i, t in enumerate(tickers):
        ...
        results.append({"Ticker": t, "Prev": prev_c, "Price": curr_p, "Chg%": ((curr_p-prev_c)/prev_c)*100, 
                        "Signal": sig, "Time": (now - timedelta(minutes=i*15)).strftime("%H:%M"), 
                        "Date": (now - timedelta(minutes=i*15)).strftime("%d/%m/%y"), "raw_t": now - timedelta(minutes=i*15)})
...
# --- 2. ส่วนแสดงผลหลัก ---
st.subheader("🛰️ Guardian: Mobile Alpha (Graph Fix)")
...
# --- 3. แก้ไขจุดที่เป็นหน้าว่าง (Graph Implementation) ---
if selected != "--- Select Ticker ---":
    with st.container():
        ...
        # ปรับปรุงการสร้างกราฟให้ Chrome Android แสดงผลได้
        ...
        fig.update_layout(
            ...
            # เพิ่มความลื่นไหลในการซูมบนมือถือ
            dragmode='pan' 
        )
        
        # ใช้คำสั่งที่เสถียรที่สุดสำหรับ Chrome Android
        st.plotly_chart(fig, use_container_width=True, config={
            'displayModeBar': False, 
            'scrollZoom': True, # อนุญาตให้ใช้สองนิ้วซูม
            'responsive': True
        })
        
        st.info(f"📌 **{selected} สรุป:** ราคางัดตัวขึ้นพร้อมวอลุ่มหนาแน่น")
