...
st.markdown("""
    ...
    /* บังคับสีตัวหนังสือในตารางให้เข้มชัดเจนที่สุด */
...
# --- 2. NAVIGATION LOGIC (Fast Response) ---
...
def fetch_and_analyze(ticker):
    try:
        ...
        df = df.dropna()
        # คำนวณอินดิเคเตอร์ตามสูตรที่คุณมิลค์ใช้ใน Pine Script
        ...
        buy_sig = (df['wt1'].shift(1) < df['wt2'].shift(1)) & (df['wt1'] > df['wt2']) & (df['wt1'] < -50) & (df['Close'] > df['ema8'])
        sell_sig = (df['wt1'].shift(1) > df['wt2'].shift(1)) & (df['wt1'] < df['wt2']) & (df['wt1'] > 48)

        sigs = df[buy_sig | sell_sig].copy()
        if not sigs.empty:
            last = sigs.iloc[-1]
            ...
            return {
                ...
                "Signal": "▲ Deep Buy" if last.name in df[buy_sig].index else "⚠️ P-Sell",
...
if st.session_state.current_page == "Thai Scan":
    ...
    tz = pytz.timezone('Asia/Bangkok')
    st.markdown(f'<div class="time-status">🕒 {datetime.now(tz).strftime("%H:%M:%S")} | Guardian V6.0 Stable</div>', unsafe_allow_html=True)
    
    # รายชื่อหุ้น SET100 + Growth
    ...
    results = [fetch_and_analyze(t) for t in full_list]
    results = [r for r in results if r]

    if results:
        df_m = pd.DataFrame(results).sort_values("raw_time", ascending=False).head(45)
        ...
        def row_color(row):
            ...
            val_c = '#059669' if m['%Chg'] > 0 else ('#dc2626' if m['%Chg'] < 0 else '#111827')
            return [f'color: {sig_c}; font-weight:bold;', 'color: #111827;', f'color: {val_c}; font-weight:bold;', f'color: {val_c};', f'color: {sig_c}; font-weight:bold;', 'color: #111827;']

        st.dataframe(df_final.style.format({"Prev":"{:.2f}","Price":"{:.2f}","%Chg":"{:.2f}%"}).apply(row_color, axis=1), 
    ...
    st.markdown("<div style='background-color:#1e293b; padding:20px; border-radius:15px; max-width:800px; margin:auto; text-align:center; color:white;'>", unsafe_allow_html=True)
    st.write(f"สวัสดีครับคุณมิลค์ (Aowat Lukthong)")
    st.write("บริษัท พอเพียง อิเล็คทริค พลัส จำกัด (PPE)")
    st.write("---")
    st.write("ระบบพร้อมสำหรับการสแกนสัญญาณ Deep Buy และ P-Sell")
    ...
    st.markdown("<h2 style='text-align:center; color:white; margin-top:50px;'>🇺🇸 US Market Scan</h2>", unsafe_allow_html=True)
    us_list = ['IONQ', 'IREN', 'SMX', 'ONDS']
    us_results = [fetch_and_analyze(t) for t in us_list]
    us_results = [r for r in us_results if r]
    if us_results:
        st.table(pd.DataFrame(us_results).drop(columns=['raw_time']))

st.write("---")
st.caption("Por Piang Electric Plus Co., Ltd. | Final Stable v6.0")
