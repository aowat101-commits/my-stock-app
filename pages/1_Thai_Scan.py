...
st.set_page_config(page_title="Guardian Original", layout="wide")
st.markdown("""
    <style>
...
# 3. Original Scanner Logic (Optimized for Mobile & Stability)
def analyze_original_guardian(ticker):
    try:
        # กลับมาใช้รายชั่วโมง (1h)
...
# 4. UI Rendering
st.subheader("🛡️ Guardian Swing (Mobile Original)")
if st.button("🔄 Full Scan (1H)", use_container_width=True): st.rerun()

@st.fragment(run_every="10m")
def runtime():
    tz = pytz.timezone('Asia/Bangkok')
    st.markdown(f'<div class="time-status">🕒 Sync: {datetime.now(tz).strftime("%H:%M:%S")} | Logic: Original (1H & WT<-45)</div>', unsafe_allow_html=True)
    
    results = []
    bar = st.progress(0, text="กำลังสแกนรายชั่วโมง...")
    for i, t in enumerate(full_scan_list):
        res = analyze_original_guardian(t)
    ...
    if results:
        ...
        st.info("ไม่พบสัญญาณตามเงื่อนไข Original (WT < -45) ในรอบ 60 วัน")
