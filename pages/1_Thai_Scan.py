...
st.markdown("""
    ...
    /* ส่วนจัดการหุ้นสีเหลืองทองสว่าง */
...
def fetch_guardian_engine(ticker, mode='Watchlist'):
    try:
        ...
        # คอลัมน์ที่ 7: เวลาขึ้นก่อนวันที่ (HH:MM DD/MM)
        ...
        # สัญญาณ Signal พร้อม Emoji (เขียว=Buy, แดง=Sell, เหลือง=P-Sell/Deep Buy)
        sig, s_col = "-", "#FFD700"
        ...
        return [ticker.replace('.BK', ''), f"{pp:.2f}", f"{cp:.2f}", f"{chg:+.2f}", f"{(chg/pp)*100:.2f}%", 
                f"{trade_val:.2f}M" if mode == 'Watchlist' else sig, last_update, 
                "#00FF00" if chg > 0 else "#FF1100", 
                s_col if mode == 'Scan' else val_col, s_col]
    except: return None
...
# --- 3. STOCK DATABASE (SET100 + MAI ยอดนิยม) ---
TH_STOCKS = [
    'PTT.BK', 'PTTEP.BK', 'DELTA.BK', 'ADVANC.BK', 'AOT.BK', 'CPALL.BK', 'SCB.BK', 'KBANK.BK', 
    'BBL.BK', 'KTB.BK', 'GULF.BK', 'GPSC.BK', 'BGRIM.BK', 'EA.BK', 'TRUE.BK', 'HANA.BK', 
    'KCE.BK', 'BDMS.BK', 'CPN.BK', 'IVL.BK', 'BANPU.BK', 'TOP.BK', 'INTUCH.BK', 'MINT.BK', 
    'CRC.BK', 'OR.BK', 'PTTGC.BK', 'SCC.BK', 'TU.BK', 'BH.BK', 'LH.BK', 'TIDLOR.BK', 'SAWAD.BK',
    'MTC.BK', 'BTS.BK', 'BEM.BK', 'CBG.BK', 'OSP.BK', 'GLOBAL.BK', 'HMPRO.BK', 'COM7.BK'
]
US_STOCKS = ['IONQ', 'NVDA', 'IREN', 'TSLA', 'SMX', 'ONDS', 'MARA', 'AMD', 'AAPL', 'MSFT', 'META']
...
tz = pytz.timezone('Asia/Bangkok')
now = datetime.now(tz)
dt_label = f"📅 {now.strftime('%d/%m/%Y | %H:%M:%S')}"
...
if p == 'Home':
    ...
    with c_img2: st.image("https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?q=80&w=1000", use_container_width=True)
    ...
    if mode == 'Scan':
        ...
        t_list = TH_STOCKS[:10] if p=="TS" else US_STOCKS[:6]
    else:
        with st.expander("➕ Manage Your Watchlist"):
            if p == 'TW': st.session_state.t_watch = st.multiselect("Select Thai Stocks:", TH_STOCKS, default=st.session_state.t_watch)
            else: st.session_state.u_watch = st.multiselect("Select US Stocks:", US_STOCKS, default=st.session_state.u_watch)
