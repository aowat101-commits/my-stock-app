...
st.markdown("""
    ...
    /* บีบทุกอย่างเข้ากึ่งกลางแบบถาวร */
...
# --- 4. PAGE LOGIC ---
if curr_p == 'Home':
    st.markdown('<h1 style="color: #FFD700;">TRADING HOME</h1>', unsafe_allow_html=True)
    ...
    st.markdown(f'<h1 style="color: #FFD700;">{"🇹🇭" if curr_m == "th" else "🇺🇸"} MENU</h1>', unsafe_allow_html=True)
    if st.button("📋 WATCHLIST"): go('Watch', curr_m)
    if st.button("🔍 MARKET SCAN"): go('Scan', curr_m)
    if st.button("🏠 กลับหน้าหลัก"): go('Home')

elif curr_p == 'Watch':
    st.markdown('<h1 style="color: #FFD700;">WATCHLIST</h1>', unsafe_allow_html=True)
    ...
    st.markdown('<h1 style="color: #FFD700;">SCAN</h1>', unsafe_allow_html=True)
    if st.button("⬅ กลับเมนูตลาด"): go('SubMenu', curr_m)
